        ...
        <?php
         session_start();
            ...
            include '../connect_db.php';
                $orders = mysqli_query($con, "SELECT dathang.name, dathang.address, dathang.phone, dathang.note, chitietdathang.*, hanghoa.name as product_name 
                    FROM dathang
                    INNER JOIN chitietdathang ON dathang.id = chitietdathang.order_id
                    INNER JOIN hanghoa ON hanghoa.id = chitietdathang.product_id
                    WHERE dathang.id = " . $_GET['id']);
