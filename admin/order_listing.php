...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $orders = mysqli_query($con, "SELECT * FROM `dathang` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $orders = mysqli_query($con, "SELECT * FROM `dathang` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    mysqli_close($con);
    ?>
